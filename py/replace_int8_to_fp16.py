...
def main():
    ...
    source_replacements = [
        ...
        ("(int8_t*)", "(fp16*)"),
        ("int8_t", "fp16"),
        ("dequantize(int8_t q, float scale)", "dequantize(fp16 q, float scale)"),
        ("return q * scale;", "return fp16_to_float(q) * scale;"),
    ]
    ...
    if os.path.exists(math_ops_path):
        ...
        dequantize_pattern = r'inline float dequantize\(fp16 q, float scale\) \{([^}]*)\}'
        dequantize_replacement = r'inline float dequantize(fp16 q, float scale) {\n    return fp16_to_float(q) * scale;\n}'
    ...
    print("Conversion from int8_t to fp16 completed successfully!")
